...
const employeeUpdater = () =>{
  for(let i = 0;i<employees.length;i++){
    for(let prop in employees[i]){
      if(employees[i][prop]==="Theo"){
        employees.splice(i,1)
      }
      if(employees[i][prop]==="Lorie"){
...
cat.catFriends[1].name[0]=fluffy2ndFriend;
...
const recordCleaner =() =>{
 ...
 for(let i = 0;i<arr.length;i++){
   for(let key in arr[i]){
     key==="atFaultForAccident"?arr[i][key]=false:false
   }
 }
}
